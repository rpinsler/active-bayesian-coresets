...
from acs.coresets import FrankWolfe, Argmax, ImportanceSampling, Random
from acs.model import NeuralLinear, NeuralLinearTB

parser = argparse.ArgumentParser()

# experiment
parser.add_argument("--save_dir", default='./experiments/active_regression', help="Save directory")
...
parser.add_argument('--acq', default='BALD', help='Active learning acquisition function (BALD, Entropy, ACS, ACS+H)')
parser.add_argument('--coreset', default='Argmax', help='Coreset construction (Argmax, Imputed, Random, Best, IS, FW)')
...
if __name__ == '__main__':
    ...
    if args.acq == 'BALD':
        acq = A.linear_bald_tb
    elif args.acq == 'Entropy':
        acq = A.linear_maxent_tb
    elif args.acq == 'ACS':
        acq = A.linear_acs_tb
        if args.coreset == 'FW':
            cs_kwargs['dotprod_fn'] = A.linear_acs_cross_tb
    elif args.acq == 'ACS+H':
        acq = A.linear_acs_ent_tb
        if args.coreset == 'FW':
            cs_kwargs['dotprod_fn'] = A.linear_acs_cross_ent_tb
    else:
        raise ValueError('Invalid acquisition function: {}'.format(args.acq))

    if args.coreset in ['Argmax', 'Imputed']:
        coreset = Argmax
    elif args.coreset == 'IS':
        coreset = ImportanceSampling
    elif args.coreset == 'FW':
        assert args.acq in ['ACS', 'ACS+H']
        coreset = FrankWolfe
    elif args.coreset == 'Random':
        coreset = Random
    elif args.coreset == 'Best':
        coreset = None  # not needed
        data.x_mean, data.x_std = data.get_statistics(data.X[data.index['train']])
    else:
    ...
    while len(data.index['train']) < args.init_num_labeled + args.budget:
        ...
        optim_params = {'num_epochs': args.training_epochs, 'batch_size': get_batch_size(args.dataset, data),
        ...
        for (x, _) in dataloader:
            ...
            feat_data.X = nl.encode(x).detach().cpu().numpy()
            cs = coreset(acq, feat_data, posterior_tb, save_dir=save_dir, **cs_kwargs)
            # cs = coreset(acq, feat_data, posterior, save_dir=save_dir, **cs_kwargs)

            if args.coreset == 'Imputed':
                true_labels = []
                for i in range(batch_size):

                    # perform sequential greedy AL
                    batch = cs.build(M=1)
                    x_query, y_true = data.move_from_unlabeled_to_train(batch)
                    true_labels.append(y_true)
                    gt.stamp('batch_selection', unique=False)

                    # impute label of queried data point using mean prediction
                    data.y[data.index['train'][-1]] = nl.get_predictions(x_query, data)[0]

                    # re-train model
                    if i == batch_size - 1:
                        continue  # skip model re-training in last iteration
                ...
                data.y[data.index['train'][-batch_size:]] = np.vstack(true_labels)
            else:
